 ...
 *
 * $Id: class.tx_damcatedit_clickmenu.php,v 1.1 2004/09/10 08:50:47 cvsrene Exp $
...
class tx_damcatedit_clickMenu {
	...
	function init($item)	{
		...
		if (!$this->cmLevel)	{
			...
			if ($this->isDBmenu)	{
				$CMcontent = $this->printDBClickMenu($params['table'], $params['uid']);
			} else {
				$CMcontent = $this->printFileClickMenu($params['file']);
			}
		} else {
				// Make 2nd level clickmenu (only for DBmenus)
			if ($this->isDBmenu)	{
				$CMcontent = $this->printNewDBLevel($params['table'], $params['uid']);
			}
		}
	...
	function printDBClickMenu($table,$uid)	{
		...
		if (is_array($this->rec) || $root)	{
			...
			if(!$root && ($BE_USER->isPSet($lCP,$table,'edit')||$BE_USER->isPSet($lCP,$table,'editcontent')))	{
				...
				if (!in_array('edit',$this->disabledItems))		$menuItems['edit']=$this->DB_editRecord($table,$uid);
				$this->editOK=1;
			}

				// New:
			if (!in_array('new',$this->disabledItems) && $BE_USER->isPSet($lCP,$table,'new'))	$menuItems['new']=$this->DB_new($table,$uid,$this->rec['pid']);
...
//			}
	...
	function DB_newWizard($table,$uid,$rec)	{
		$newContentWizScriptPath = t3lib_extMgm::extRelPath('dam_catedit').'mod1/index.php';

		$url = $newContentWizScriptPath.'?id='.$rec['pid'].'&sys_language_uid='.intval($rec['sys_language_uid']);
		return $this->linkItem(
...
'.(!$this->doDisplayTopFrameCM()?'hideCM();':'')
	...
	/**
	 * Enabling / Disabling items based on list provided from GET var ($this->enDisItems)
	 *
	 * @param	array		Menu items array
	 * @return	array		Menu items array, processed.
	 */
	function enableDisableItems($menuItems)	{
		if ($this->enDisItems)	{
			...
			if (substr($this->enDisItems,0,1)=='+')	{
				$this->enDisItems = substr($this->enDisItems,1);
				$only = TRUE;
			} else {
				$only = FALSE;
			}
			...
			if ($only)	{	// Transfer ONLY elements which are mentioned (or are spacers)
				...
				foreach($menuItems as $key => $value)	{
					if (t3lib_div::inList($this->enDisItems, $key) || (is_string($value) && $value=='spacer'))	{
						$newMenuArray[$key] = $value;
					}
				}
				$menuItems = $newMenuArray;
			} else {	// Traverse all elements except those listed (just unsetting them):
				$elements = t3lib_div::trimExplode(',',$this->enDisItems,1);
...
// Include extension?
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam_catedit/lib/class.tx_damcatedit_clickmenu.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/dam_catedit/lib/class.tx_damcatedit_clickmenu.php']);
}
